...
\vskip5pt Contoh: $x^3, x^3-2, x^3+1$ adalah integral tak tertentu dari $f(x)=3x^2$, dituliskan $\int 3x^2 dx= x^3 + C$
\vskip10pt
...
$$D\Bigl\{e^{ax}\dfrac{1}{(D+a)}V\Bigr\} = e^{ax} V$$
...
Bukti:\\
$D(x~U) = x ~DU + U$      Misal : $DU = V \rightarrow U= \dfrac{1}{D}V$
...
$D^{(n)} = UD^nV + nDUD^{n-1}V+\dfrac{1}{2!} n(n-1)D^2UD^{n-2}V+\dfrac{1}{3!} n(n-1)(n-2)D^3UD^{n-3}V+..$\\
\vskip10pt
Dengan memasang n=-1, maka: \\
$\dfrac{1}{D}(UV)=U\dfrac{1}{D}V-DU\dfrac{1}{D^2}V+D^2U\dfrac{1}{D^3}V-D^3U\dfrac{1}{D^4}V+...$
...
\item Jika $N(x) = (ax^2+bx+c)$ dengan $D<0$ maka dibawa ke bentuk $\dfrac{A}{cx+d} + \dfrac{Bx+C}{ax^2+bx+c}$.
\end{enumerate}
\begin{block}{Contoh:}
\begin{enumerate}
...
\item $\cos^2x =\dfrac{1}{2}(1+\cos{2x})$
...
Bentuk: \fbox{$\int \sin \alpha x \cos \beta x dx; ~ \int \cos \alpha x \sin \beta x dx; ~ \int \sin \alpha x \sin \beta x dx$}
\begin{center}
...
\vskip3pt Subtitusi : $\tan\dfrac{x}{2}=t \Longrightarrow \dfrac{x}{2} = \arctan {t}, \rightarrow x= 2 \arctan{t}$\\
...
\vskip5pt Subtitusi: $\tan{x} = t \Longrightarrow x=\arctan{t} \rightarrow \cos{x}=\dfrac{1}{\sqrt{1+t^2}}$
...
\item $I=\int \dfrac{dx}{\sin^2x-\sin x \cos x}$
