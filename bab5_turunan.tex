...
Dengan menggunakan definisi turunan  $\Bigl( \dfrac{df}{dx}=\lim_{h\to 0}\dfrac{\Delta{f}}{\Delta{x}}=\dfrac{f(x+h)-f(x)}{h}\Bigr)$, dapatkan turunan dari fungsi berikut:
\begin{enumerate}
\item $f(x) = x^2$
\item $f(x) = x^3$
\item $f(x) = 2x$
...
Sifat:
$$^a\log{b}=\dfrac{\log{b}}{\log{a}}=\dfrac{^2\log{b}}{^2\log{a}}=\dfrac{^n\log{b}}{^n\log{a}}$$
Bilangan natural/alam= e = 2.71828183 \\
$$^a\log{b}=\dfrac{\log{b}}{\log{a}}=\dfrac{^e\log{b}}{^e\log{a}}=\dfrac{\ln{b}}{\ln{a}}$$
$$^e \log {x} = \ln {x}$$
...
\item $y= \ln | x + \sqrt{x^2 \pm a^2} | \Longrightarrow y'= \dfrac{1}{\sqrt{x^2\pm a^2}}$
\seti
\end{enumerate}
\end{frame}
...
\item Setiap suku dalam $f(x,y)=0$ diturunkan terhadap x. Karena $y$ fungsi $x$ maka setiap kali menurunkan $y$ harus digandakan dengan $y'$, kemudian hubungan yang didapat diselesaikan ke $y'$.\\
      ...
      $3(ax-y^2)y'=3(x^2-ay) \Longrightarrow y'=\dfrac{x^2-ay}{ax-y^2}$
\end{enumerate}
\end{frame}
