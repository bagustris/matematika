...
\begin{enumerate}
    \conti
    \item ${^{a}\log{b}}=\frac{^{P}\log{b}}{^{P}\log{a}}$
    \item $^{a}\log{b}^{b}\log{c}=^{a}\log{c}$
    \item $^{a}\log{b} \cdot {^{b}\log{a}}=1$
    \item $^{a}\log{\sqrt[n]{b}}=\frac{1}{n} \cdot {^{a}\log{b}}$
...
\begin{center}
\fbox{$\underset{n\rightarrow\infty}{\lim}\left(1+\frac{1}{n}\right)^n=e=2.7182818...$}
\end{center}
\begin{itemize}
\item Log dengan bilangan pokok e disebut logaritma natural
$^{e}\log x = \ln x$ dibaca lon x; $\ln e=1; \ln 1 = 0$
\item Hubungan $\log$ dengan $\ln$:
\vskip12pt
\fbox{$\log x = 0.4343 \ln x; \ln x= 2.3026 \log x$}
\end{itemize}
\end{frame}
...
\vskip12pt
\begin{itemize}
\item Bilangan kompleks: $z=a+bi$; $a=$ bagian real dari bilangan komplek z,
...
\vskip10pt
$x_{1,2}=\cfrac{-1\pm\sqrt{1^2-4(1)}}{2}=\cfrac{-1\pm\sqrt{-3}}{2}=\cfrac{-1\pm i\sqrt{3}}{2}$
$x_{1}=\cfrac{-1}{2}+\cfrac{i\sqrt{3}}{2}$
\hskip40pt
$x_{2}=\cfrac{-1}{2}-\cfrac{i\sqrt{3}}{2}$
\end{itemize}
\end{center}
\end{frame}
...
\fbox{$\cfrac{x^2}{a^2}+\cfrac{y^2}{b^2}=1$}  \hskip10pt 2a = Sb. panjang\\
...
\vskip5pt
Maka persamaan hyperbola: \fbox{$\cfrac{x^2}{a^2}-\cfrac{y^2}{b^2} = 1$}\\
\vskip5pt
Jika pusat hyperbola ($\alpha, \beta$) dan sumbu-sumbu simetri tetap $//$ sumbu x dan sumbu y; maka persamaan hiperbolanya:\\
\begin{center}
$\cfrac{(x-\alpha)^2}{a^2} - \cfrac{(y-\beta)^2}{b^2} = 1$\\
\end{center}
...
\vskip10pt
\begin{exampleblock}{Contoh:}
1 radian = $\cfrac{360}{2\pi} = \cfrac{360^o}{2(3.14159)}=57^o$ 17': 45"
\end{exampleblock}
\end{frame}
